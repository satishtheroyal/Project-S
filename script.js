...
document.addEventListener("DOMContentLoaded", function () {
        ...
        if (!storedData) {
            ...
            document.getElementById("tablesContainer").innerHTML = "<p>Error: No student data found. Please log in again.</p>";
            return;
        }
        ...
        updateResultsPage({ rollno: htno, name, branch, yearValue });
        ...
        function calculateCGPA() {
            let totalCredits = 0, totalWeightedSgpa = 0;
            Object.keys(semesterSgpa).forEach((sem) => {
                totalWeightedSgpa += semesterSgpa[sem] * semesterCredits[sem];
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                if (!data || typeof data !== "object") throw new Error("Invalid API data format");
                const container = document.getElementById("tablesContainer");
                container.innerHTML = "";
                console.log(data);
                let rowContainer = document.createElement("div");
                ...
                let tableCount = 0;
                let currentYear = 1;
                Object.keys(data).forEach((semesterKey, index) => {
                    const semesterData = data[semesterKey];
                    //jashuva
                    console.log(semesterData);
                    console.log(index);
                    ...
                    table.innerHTML = `
                        ...
                        <thead>
                            <tr><th colspan="6">Semester ${index + 1}</th></tr>
                            <tr><th>S.No</th><th>Subject Name</th><th>Subject Code</th><th>Grade</th><th>Status</th><th>Credits</th></tr>
                        </thead>
                        <tbody></tbody>
                    `;
                    ...
                    semesterData.forEach((row, idx) => {
                        const tr = document.createElement("tr");
                        tr.innerHTML = `<td>${idx + 1}</td><td>${row.Subname}</td><td>${row.Subcode}</td><td>${row.Grade}</td><td>${row.Grade !== 'F' ? 'Pass' : 'Fail'}</td><td>${row.Credits}</td>`;
                        tbody.appendChild(tr);
                    });
                    ...
                    const semesterContainer = document.createElement("div");
                    semesterContainer.className = "w-1/2 bg-gray-200 p-2 rounded-lg shadow-md";
                    semesterContainer.append(table, sgpaDisplay);
            ...
            .catch(error => {
                ...
                document.getElementById("tablesContainer").innerHTML = `<p class="text-red-600">Error: ${error.message}</p>`;
            });
    });
